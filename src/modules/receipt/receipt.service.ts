...
export class ReceiptService {
    ...
    constructor() {
        this.client = new vision.ImageAnnotatorClient({
            keyFilename: process.env.GOOGLE_JSON_KEY_PATH || './config/google-cloud-api-key.json',
        });
    }
    ...
    private parseTotalAmount(text: string): number | null {
        const totalRegex = /(TOTAL|Total|total|AMOUNT|Amount|amount|Tổng cộng|Tổng Cộng|TỔNG CỘNG|Thành tiền|Thành Tiền|THÀNH TIỀN)\s*[:\-\s]*([\d,.]+)/gi;
    ...
    private parsePaymentTime(text: string): string | null {
        const timeRegex = /(\d{1,2}[/-]\d{1,2}[/-]\d{2,4}([| ]\d{1,2}:\d{2}(:\d{2})?)?)/i;
    ...
    private parseSeller(text: string): string | null {
        const sellerKeywords = ['Seller', 'Vendor', 'Merchant', 'Người bán', 'Cửa hàng'];
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (const keyword of sellerKeywords) {
                if (line.toLowerCase().includes(keyword.toLowerCase())) {
                    const sellerMatch = line.match(new RegExp(`${keyword}\\s*[:\\s]*(.+)`));
                    if (sellerMatch) {
